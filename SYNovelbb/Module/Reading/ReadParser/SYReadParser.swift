...
/// 解析完成
typealias SYParserCompletion = (_ readModel: SYReadModel?) -> Void

class SYReadParser: NSObject {
    ...
    /// - Returns: 内容分页列表
    @objc class func pageing(attrString:NSAttributedString, rect:CGRect, isFirstChapter:Bool = false) -> [SYReadPageModel] {
...
//        }
        ...
        if !ranges.isEmpty {
            ...
            for i in 0..<count {
                ...
                }else if pageModel.headType == .paragraph { pageModel.headTypeHeight = SYReadConfigure.shared().paragraphSpacing
                    
                }else{ pageModel.headTypeHeight = SYReadConfigure.shared().lineSpacing }
