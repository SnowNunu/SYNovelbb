//
//  SYReadViewScrollController.swift
//  SYNovelbb
//
...
class SYReadViewScrollController: SYViewController, UITableViewDelegate, UITableViewDataSource {

    /// 当前主控制器
    weak var vc: SYReadController!
    
    /// 顶部状态栏
    private var topView: SYReadViewStatusTopView!
    
    /// 阅读视图
    private var tableView: SYTableView!
    
    /// 底部状态栏
    private var bottomView:SYReadViewStatusBottomView!
    ...
    private var chapterModels:[String: SYReadChapterModel] = [:]
...
//        }
    ...
    /// 获取章节内容模型
    private func GetChapterModel(chapterID:NSNumber) -> SYReadChapterModel? {
        
        var chapterModel: SYReadChapterModel? = nil
        ...
        if chapterModels.keys.contains(chapterID.stringValue) { // 内存中存在章节内容
            ...
            // 检查是否存在章节内容
            let isExist = SYReadChapterModel.isExist(bookID: vc.readModel.bookID, chapterID: chapterID)
...
//                }else{
//
//                    chapterModel = SYReadChapterModel.model(bookID: vc.readModel.bookID, chapterID: chapterID)
//                }
//
...
//            }
    ...
    /// 预加载上一个章节
    private func preloadingPrevious(_ chapterModel: SYReadChapterModel!) {
...
//
//                // 章节内容
//                var tempChapterModel:SYReadChapterModel!
//
//                // 获取章节数据
//                if !isExist {
//
...
//                }else{
//
//                    tempChapterModel = SYReadChapterModel.model(bookID: bookID, chapterID: chapterID!)
//                }
//
//                // 加入阅读内容列表
...
////
////                        // 解析章节数据
////                        let tempChapterModel = SYReadChapterModel(data)
////
////                        // 章节类容需要进行排版一篇
////                        tempChapterModel.content = SYReadParser.contentTypesetting(content: chapterModel.content)
////
////                        // 保存
...
//            }
    ...
    /// 预加载下一个章节
    private func preloadingNext(_ chapterModel: SYReadChapterModel!) {
